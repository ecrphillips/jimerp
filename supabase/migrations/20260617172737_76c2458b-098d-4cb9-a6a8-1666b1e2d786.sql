...
BEGIN
  ...
  -- Ghost ship_picks: no fg_inventory_log entry for that product within +/- 2h
  ...
  WHERE NOT EXISTS (
    ...
    WHERE fil.product_id = oli.product_id
      AND fil.created_at BETWEEN sp.created_at - interval '2 hours'
                             AND sp.created_at + interval '2 hours'
  );
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('ship_picks', v_rows);

  -- Ghost packing_runs: no fg_inventory_log entry for that product within +/- 2h
  ...
  -- Ghost roasted_batches: no wip_ledger row references them
