...
--
-- Replaces multi-step client-side write sequences with single SECURITY DEFINER
-- functions so batch status, ledger entries, and lot consumption commit (or
-- roll back) as a unit. Fixes:
--   * blend execute: partial ledger insert after batches marked consumed
--   * pack update: packing_runs / inventory_transactions divergence + stale
--     previous-units race from rapid clicks
--   * mark roasted: batch flipped ROASTED but ROAST_OUTPUT ledger row missing
--   * revert to planned: status reverted without the reversing adjustment
--
-- All functions are gated to ADMIN/OPS via public.has_role.

-- ============================================================
-- Helper: assert caller is internal staff (ADMIN or OPS)
-- ============================================================
...
-- ============================================================
-- 1) execute_blend
--    Atomically consume component batches and write balanced
--    ledger rows (positive blend output + negative component
--    decrements). Row locks prevent two concurrent blends from
...
BEGIN
  ...
  -- Lock the candidate batches. Any batch already consumed (or not ROASTED)
  -- will be missing from the locked set and we abort before writing anything.
  ...
  FOR i IN 1 .. array_length(p_batch_ids, 1) LOOP
    ...
    -- Negative ADJUSTMENT: remove blended kg from the component group's WIP
  ...
  -- Positive ADJUSTMENT: blend output enters the parent blend group's WIP
  INSERT INTO public.inventory_transactions
    (transaction_type, roast_group, quantity_kg, is_system_generated, created_by, notes)
  VALUES
...
-- ============================================================
-- 2) update_packing_units
--    Atomically upsert packing_runs and write the WIP/FG ledger
--    rows for the delta. The previous unit count is read from the
--    DB under lock, so rapid concurrent clicks cannot lose deltas.
--    The WIP availability guard runs server-side against the same
--    ledger the UI treats as authoritative.
-- ============================================================
...
BEGIN
  ...
  -- Serialize pack updates per roast group so concurrent packs cannot both
  -- pass the availability check and jointly overdraw WIP.
  ...
  IF v_delta > 0 AND p_roast_group IS NOT NULL AND v_kg_delta > 0 THEN
    SELECT COALESCE((
        SELECT sum(quantity_kg) FROM public.inventory_transactions
        WHERE roast_group = p_roast_group
          AND transaction_type IN ('ROAST_OUTPUT', 'PACK_CONSUME_WIP', 'ADJUSTMENT', 'LOSS')
      ), 0)
      + COALESCE((
        SELECT sum(kg_delta) FROM public.wip_adjustments
        WHERE roast_group = p_roast_group
      ), 0)
    INTO v_available;

    IF v_kg_delta - v_available > 0.001 THEN
      RAISE EXCEPTION 'Not enough WIP for %: need % kg, only % kg available. Coffee may already be committed to a blend.',
        p_roast_group, round(v_kg_delta, 2), round(v_available, 2);
    END IF;
  END IF;
...
-- ============================================================
-- 3) mark_batch_roasted
--    Atomically flip a PLANNED batch to ROASTED, write the
--    ROAST_OUTPUT ledger row, optionally record a LOSS, and
--    optionally consume from a green lot (validated under lock).
--    Returns true if the batch transitioned, false if it was
--    already roasted (no-op).
-- ============================================================
...
BEGIN
  ...
  IF v_batch.status <> 'PLANNED' THEN
    RETURN false; -- already roasted; nothing written
  END IF;
...
-- ============================================================
-- 4) revert_batch_to_planned
--    Atomically flip a ROASTED batch back to PLANNED and write
--    the reversing negative ADJUSTMENT for its recorded output.
--    Returns true if reverted, false if the batch was not ROASTED.
-- ============================================================
