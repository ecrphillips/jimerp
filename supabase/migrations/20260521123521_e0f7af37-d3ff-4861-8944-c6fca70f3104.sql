...
-- Admin uploads a CSV per run; clients/members view the latest published run.
-- ============================================================

-- Parent: one row per audit run (one date = one run)
...
-- Child: one row per scanned product
...
-- Runs: admins full; everyone else can SELECT published runs only.
...
-- Rows: admins full; everyone else can SELECT rows belonging to published runs.
...
--   Atomically create a draft run + insert rows.
--   _rows JSON shape: [{brand,product_name,product_url,bag_size_g,price_cad,price_per_g_cad,status,notes}, ...]
--   Replaces any existing DRAFT run with the same run_date.
...
BEGIN
  ...
  -- If a draft already exists for this date, replace it. Refuse if it's published.
...
--   Mark a run as published. Auto-unpublish all other runs so exactly one is live.
...
-- delete_market_price_audit (drafts only)
