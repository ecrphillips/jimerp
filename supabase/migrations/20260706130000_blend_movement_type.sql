-- Blends get their own inventory movement type: BLEND (was ADJUSTMENT).
--
-- Why: blend activity was recorded as ADJUSTMENT, polluting the Adjustments
-- figure on the WIP screen with blend movement that should show separately.
--
-- 1. Add 'BLEND' to the inventory_transaction_type enum.
-- 2. Rewrite execute_blend so both sides it writes use BLEND instead of
--    ADJUSTMENT: the component roast groups (negative kg) and the destination
--    blend roast group (positive kg). Weight math is unchanged — only the
--    movement-type label on the rows changes.
--
-- Existing ledger rows are NOT touched; this applies to blends created from now on.
--
-- NOTE: ADD VALUE appends a new enum label. It cannot be *used* in the same
-- transaction it is added, but CREATE OR REPLACE FUNCTION does not use it at
-- definition time (the label is only cast at runtime), so the two statements
-- are safe to run together. If your SQL editor still objects, run the ALTER TYPE
-- statement first on its own, then the function.

ALTER TYPE public.inventory_transaction_type ADD VALUE IF NOT EXISTS 'BLEND';

CREATE OR REPLACE FUNCTION public.execute_blend(p_blend_roast_group text, p_blend_display_name text, p_batch_ids uuid[], p_consume_kgs numeric[])
 RETURNS numeric
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR i IN 1 .. array_length(p_batch_ids, 1) LOOP
    ...
    VALUES
      ('BLEND', v_batch.roast_group, -p_consume_kgs[i], true, auth.uid(),
  ...
  INSERT INTO public.inventory_transactions
    (transaction_type, roast_group, quantity_kg, is_system_generated, created_by, notes)
  VALUES
    ('BLEND', p_blend_roast_group, v_total, true, auth.uid(),
