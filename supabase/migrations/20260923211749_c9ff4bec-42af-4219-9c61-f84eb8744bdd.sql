...
-- Separate from coroast_bookings / coroast_loring_blocks: no billing period,
-- no hour-ledger writes; each resource is fully independent.
-- Double booking is prevented by a trigger that serialises writes per
-- (resource, date) with an advisory lock.
