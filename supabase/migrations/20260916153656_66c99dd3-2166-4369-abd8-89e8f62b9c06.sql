ALTER TABLE public.roasted_batches ADD COLUMN IF NOT EXISTS roasted_at timestamptz;

UPDATE public.roasted_batches SET roasted_at = updated_at WHERE status = 'ROASTED' AND roasted_at IS NULL;
...
BEGIN
  ...
  SET status = 'ROASTED', actual_output_kg = p_actual_output_kg, roasted_at = now()
  WHERE id = p_batch_id;
...
BEGIN
  ...
  END IF;
  UPDATE public.roasted_batches
  SET status = 'PLANNED', roasted_at = NULL
  WHERE id = p_batch_id;
