-- Account Owner Self-Service Management RPCs and supporting RLS policies.
...
BEGIN
  PERFORM public._assert_account_owner(p_account_id);
  SELECT user_id, is_owner INTO v_target_user_id, v_target_was_owner
    FROM public.account_users
   WHERE id = p_account_user_id AND account_id = p_account_id;
  ...
  IF v_target_was_owner AND NOT p_is_owner THEN
    SELECT COUNT(*) INTO v_active_owner_count
      FROM public.account_users
     WHERE account_id = p_account_id AND is_owner = true AND is_active = true AND id <> p_account_user_id;
    IF v_active_owner_count < 1 THEN
      RAISE EXCEPTION 'Cannot remove the last owner from the account';
    END IF;
  END IF;
  SELECT programs INTO v_programs FROM public.accounts WHERE id = p_account_id;
  ...
  IF p_location_access = 'ASSIGNED' AND COALESCE(array_length(p_assigned_location_ids, 1), 0) > 0 THEN
    SELECT lid INTO v_orphan_location
     ...
     WHERE NOT EXISTS (SELECT 1 FROM public.account_locations al WHERE al.id = lid AND al.account_id = p_account_id)
     LIMIT 1;
  ...
  SET is_owner = p_is_owner, can_place_orders = p_can_place_orders, can_book_roaster = p_can_book_roaster,
      can_manage_locations = p_can_manage_locations, can_invite_users = p_can_invite_users,
      location_access = p_location_access, updated_at = now()
  WHERE id = p_account_user_id AND account_id = p_account_id;
  DELETE FROM public.account_user_locations WHERE account_user_id = p_account_user_id;
  IF p_location_access = 'ASSIGNED' AND COALESCE(array_length(p_assigned_location_ids, 1), 0) > 0 THEN
...
BEGIN
  PERFORM public._assert_account_owner(p_account_id);
  SELECT user_id, is_owner, is_active INTO v_target_user_id, v_target_is_owner, v_target_is_active
    FROM public.account_users
   WHERE id = p_account_user_id AND account_id = p_account_id;
  IF v_target_user_id IS NULL THEN
    RAISE EXCEPTION 'User not found in this account';
  ...
  IF v_target_is_owner AND v_target_is_active THEN
    ...
    END IF;
  END IF;
  UPDATE public.account_users SET is_active = false, updated_at = now()
   WHERE id = p_account_user_id AND account_id = p_account_id;
END;
$$;
...
BEGIN
  ...
  SELECT (is_owner OR can_manage_locations) INTO v_caller_can_manage
    FROM public.account_users
   WHERE account_id = p_account_id AND user_id = auth.uid() AND is_active = true;
  IF NOT COALESCE(v_caller_can_manage, false) THEN
    RAISE EXCEPTION 'Not authorized to manage locations for this account';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM public.accounts
     WHERE id = p_account_id AND 'MANUFACTURING' = ANY(COALESCE(programs, ARRAY[]::text[]))
  ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM public.account_locations
     WHERE account_id = p_account_id AND UPPER(location_code) = v_clean_code
  ) THEN
    RAISE EXCEPTION 'A location with code % already exists for this account', v_clean_code;
  END IF;
  INSERT INTO public.account_locations (account_id, location_name, location_code, address, qbo_billing_entity, is_active)
  VALUES (p_account_id, TRIM(p_location_name), v_clean_code,
          NULLIF(TRIM(COALESCE(p_address, '')), ''),
          NULLIF(TRIM(COALESCE(p_qbo_billing_entity, '')), ''), true)
